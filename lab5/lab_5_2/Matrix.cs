...
namespace labwork_5_2
{
    class Matrix
    {
        ...
        private Matrix CreateMatrixWithoutRow(int row)
        {
            if (row < 0 || row >= Rows)
            {
                Console.WriteLine("Помилка індекса строки");
            }
        ...
        public double CalculateDeterminant()
        {
            if (IsSquare() == false)
            {
                ...
                Console.WriteLine("Детермінант обчислюється тільки для квадратної матриці");
                Console.ResetColor();
        ...
        public Matrix MultiplyMatrix(Matrix matrix2)
        {
            ...
            {
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine("Дані матриці можна перемножити");
                Console.ResetColor();
                Matrix result = new Matrix(0, 0);
                return result;
            }
